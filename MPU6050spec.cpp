...
//
void MPU6050class::Setup(byte escalaAcelerometro, byte escalaGiroscopio)
{
	Serial.begin(9600);
	Wire.begin();
	Serial.println("MPU6050 is online...");
	Serial.println("Selfteste");
	MPU6050SelfTest(SelfTest); // Start by performing self test and reporting values
	Serial.println("End Selfteste");
...
void MPU6050class::readAccelData(int16_t * destination)
{
...
void MPU6050class::readGyroData(int16_t * destination)
{
...
int16_t MPU6050class::readTempData()
{
...
void MPU6050class::LowPowerAccelOnlyMPU6050()
{
...
void MPU6050class::calibrateMPU6050(float * dest1, float * dest2)
{
...
void MPU6050class::MPU6050SelfTest(float * destination) // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
{
	uint8_t rawData[4];
	uint8_t selfTest[6];
	float factoryTrim[6];
	...
	Serial.println("inicio1");																					//  Output self-test results and factory trim calculation if desired
	...
	Serial.println("inicio2");																					// To get to percent, must multiply by 100 and subtract result from 100
	for (int i = 0; i < 6; i++) {
...
void MPU6050class::writeByte(uint8_t address, uint8_t subAddress, uint8_t data)
{
...
uint8_t MPU6050class::readByte(uint8_t address, uint8_t subAddress)
{
...
void MPU6050class::readBytes(uint8_t address, uint8_t subAddress, uint8_t count, uint8_t * dest)
{
	Wire.beginTransmission(address);   // Initialize the Tx buffer
	Wire.write(subAddress);            // Put slave register address in Tx buffer
	Wire.endTransmission(false);       // Send the Tx buffer, but send a restart to keep connection alive
...
void MPU6050class::MadgwickQuaternionUpdate(float ax, float ay, float az, float gx, float gy, float gz)
{
...
float MPU6050class::leYaw()
{
	return yaw;
}

float MPU6050class::lePitch()
{
	return pitch;

}

float MPU6050class::leRoll()
{
	return roll;
